function checkForVisibleIntersect(path, canX, canY, CAN_WIDTH, CAN_HEIGHT, scale) {
    ...
    if ( //Check if the line would cut across a corner of the visible canvas
    ...
    else if ( //Check if the line would stretch across the visible canvas
        (((Math.min(x0, x1)) <= leftVisExtent && Math.max(x0, x1) <= rightVisExtent)
            && (Math.min(y0, y1) <= topVisExtent && Math.max(y0, y1) >= bottomVisExtent))
        || (((Math.min(y0, y1)) >= topVisExtent && Math.max(y0, y1) <= bottomVisExtent)
            && (Math.min(x0, x1) <= leftVisExtent && Math.max(x0, x1) >= rightVisExtent))
    ) return true;
